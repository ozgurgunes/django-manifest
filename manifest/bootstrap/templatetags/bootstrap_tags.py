...
from manifest.bootstrap import defaults
...
def bootstrap_horizontal(element, label_cols={}):
    ...
    for cl in label_cols.split(' '):
        ...
        try:
            ...
            value_nb_cols = defaults.BOOTSTRAP_GRID_COLUMNS

        if value_nb_cols >= defaults.BOOTSTRAP_GRID_COLUMNS:
            splited_class[-1] = defaults.BOOTSTRAP_GRID_COLUMNS
        else:
            offset_class = cl.split('-')
            offset_class[-1] = 'offset-' + str(value_nb_cols)
            splited_class[-1] = str(defaults.BOOTSTRAP_GRID_COLUMNS - value_nb_cols)
